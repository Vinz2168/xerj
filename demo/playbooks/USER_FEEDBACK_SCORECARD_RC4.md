...
- Binary/RaBitQ (BBQ) quantization unimplemented; `bbq_*`/`int4_*` mappings silently keep full f32 (no memory saving) — HNSW RAM wall NOT mitigated by SQ8 either: `scalar8` kNN scores from ingest-time 1-byte/dim codes ([#392](https://github.com/xerj-org/xerj/issues/392)), which shrinks the scoring working set ~4×, but `_source` keeps the f32 originals and the HNSW graph still holds f32. No DiskANN.
